...
int main() {
    ...
    while(1) {
        ...
        int slider_value = lv_slider_get_value(objects.obj1); // Get the current slider value
        lv_label_set_text_fmt(objects.obj2, "  HwIT: %d %%", slider_value); // Update the label with the slider value
        ...
        if (btna == true || btnb == true) 
        {
            ...
            { 
                lv_slider_set_value(objects.obj1, slider_value, LV_ANIM_OFF); // Set the slider value without animation
